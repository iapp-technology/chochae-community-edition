...
const log = new log_1.default('naiveBaye.ts');
log.enable = true;
                ...
                for (const i in this.intents) {
                    if (this.intents.hasOwnProperty(i)) {
                        for (let l = 0; l < seq[k].length; l++) {
                            ...
                            if (q.length > 0) {
                                try {
                                    const foundInClass = (this.concat[i].match(new RegExp(q, 'g')) || []).length;
                                    const foundInAll = (this.all.match(new RegExp(q, 'g')) || [])
                                        .length;
                                    log.p('Query:', q, 'Found in Class:', foundInClass, 'Found in All:', foundInAll);
                                    if (foundInAll > 0) {
                                        score[i].push(Math.log10((foundInClass * nk * nk + EPSILON) / foundInAll));
                                    }
                                }
                                catch (e) {
                                    log.f('Exception:', e);
...
exports.default = NaiveBaye;
//# sourceMappingURL=naiveBaye.js.map